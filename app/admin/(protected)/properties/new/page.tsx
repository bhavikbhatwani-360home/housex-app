...
export default function NewProperty() {
  ...
  const [f, setF] = useState({
    name: "", developer: "", city: "Mumbai (MMR)", locality: "", bhk: "2 BHK",
    facing: "East", carpetSqft: "", distanceToStationM: "", reraId: "", status: "Live",
    amenities: "", brochureUrl: "",
  });
  ...
  return (
    <div>
      <header className="h-14 border-b border-hx-line bg-white flex items-center px-6 gap-3 sticky top-0 z-10">
        ...
        <button form="propform" disabled={busy} className="ml-auto h-9 px-4 rounded-lg bg-hx-red text-white text-[13px] font-semibold shadow-hx-red disabled:opacity-40">
          {busy ? "Saving…" : "Publish"}
        </button>
      </header>
      ...
      <div className="p-6 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 max-w-5xl">
        {/* form */}
        <form id="propform" onSubmit={submit} className="space-y-5">
          <Card title="Basics">
            <Grid>
              <Input label="Project name" value={f.name} onChange={set("name")} placeholder="Greenvalley" required />
              <Input label="Developer" value={f.developer} onChange={set("developer")} placeholder="Square Homes" required />
        ...
        <div className="lg:sticky lg:top-20 h-fit">
          ...
          <div className="rounded-xl border border-hx-line bg-white p-4">
            ...
            <div className="text-[12px] text-hx-muted">{f.developer || "Developer"} · {f.locality || "Locality"}</div>
            ...
            <div className="mt-3 pt-3 border-t border-hx-line text-[12px] text-hx-slate">
              {prices.length} unit{prices.length === 1 ? "" : "s"}
              {f.reraId && <> · RERA {f.reraId}</>}
            </div>
            <p className="mt-3 text-[11.5px] text-hx-muted leading-relaxed">
              When you publish, Baba can immediately recommend this and quote its real floor-wise prices to buyers.
            </p>
          </div>
        </div>
      </div>
