...
async function getVisits() {
  try {
    ...
    return { visits, dbError: false };
  } catch {
    return { visits: [], dbError: true };
  }
}
        ...
        {dbError ? (
          <div className="rounded-xl border border-hx-line bg-white p-5 flex items-start gap-3 max-w-xl">
            ...
            <div>
              ...
              <p className="text-[13px] text-hx-muted mt-1">Booked visits will appear here once the database is connected.</p>
            </div>
          </div>
        ) : visits.length === 0 ? (
          <div className="rounded-xl border border-hx-line bg-white p-10 text-center max-w-xl mx-auto">
            ...
            <div className="text-[15px] font-semibold">No visits booked yet</div>
            <p className="text-[13px] text-hx-muted mt-1">When a buyer books a site visit in chat, it shows up here.</p>
          </div>
        ) : (
                  ...
                  <tr key={v.id} className="border-b border-hx-line last:border-0 hover:bg-hx-bg/60">
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center gap-1.5 text-[12.5px] text-hx-slate">
                        {v.mode === "Video" ? <Video className="w-3.5 h-3.5" /> : <Building2 className="w-3.5 h-3.5" />}
                        {v.mode}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold bg-hx-warning/10 text-hx-warning">{v.status}</span>
                    </td>
