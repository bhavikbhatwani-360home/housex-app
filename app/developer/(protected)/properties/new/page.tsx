...
export default function DevNewProperty() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/developer/properties", {
        method: "POST",
  ...
  return (
    <div>
      <header className="h-14 border-b border-hx-line bg-white flex items-center px-6 gap-3 sticky top-0 z-10">
        ...
        <button form="devpropform" disabled={busy} className="ml-auto h-9 px-4 rounded-lg bg-hx-red text-white text-[13px] font-semibold shadow-hx-red disabled:opacity-40">{busy ? "Saving…" : "Publish"}</button>
      </header>

      <div className="p-6 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 max-w-5xl">
        <form id="devpropform" onSubmit={submit} className="space-y-5">
          <Card title="Basics">
            <Grid>
        ...
        <div className="lg:sticky lg:top-20 h-fit">
          ...
          <div className="rounded-xl border border-hx-line bg-white p-4">
            ...
            <div className="mt-2 flex items-center gap-2 text-[13px]">
              ...
              <span className="text-hx-muted">·</span><span>{f.bhk}</span>
